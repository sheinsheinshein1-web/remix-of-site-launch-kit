...
const MobileTabBar = ({ ctaLabel, onCtaClick }: { ctaLabel?: string; onCtaClick?: () => void }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden flex justify-center pb-[max(env(safe-area-inset-bottom),4px)]">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className="flex-1 flex items-center justify-center py-2.5 px-6"
              >
                <tab.icon
